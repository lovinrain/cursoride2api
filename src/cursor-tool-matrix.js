...
const EXEC_RUNTIME_TOOLS = [
  ...
  { field: 'deleteArgs', source: 'cursor-native-exec', status: 'passthrough-optional', bridgeTool: 'Bash', reason: 'Bridged through a guarded local script and returned as native deleteResult.' },
  ...
  { field: 'fetchArgs', source: 'cursor-native-exec', status: 'server-fetch-default', bridgeTool: 'WebFetch', reason: 'Server-side WebFetch is enabled by default with public-URL guards; set CURSOR_SERVER_WEBFETCH=0 to disable. Broad search should use Cursor backend WebSearch.' },
...
const ARG_SCHEMA_TO_SOURCE = [
  ...
  { schema: 'DeleteArgsSchema', tool: 'Delete', source: 'cursor-native-exec', status: 'passthrough-optional' },
  ...
  { schema: 'FetchArgsSchema', tool: 'Fetch', source: 'cursor-native-exec', status: 'server-fetch-default', bridgeTool: 'WebFetch', reason: 'Server-side WebFetch is enabled by default with public-URL guards; set CURSOR_SERVER_WEBFETCH=0 to disable.' },
...
function getCursorToolMatrix(options = {}) {
  ...
  return {
    ...
    explanation: {
      ...
      webLookupPolicy: 'By default, client-declared WebSearch/Search is not forwarded into RequestContext.tools because broad search should use Cursor native WebSearch. WebFetch/Fetch is server-side enabled by default for explicit URL fetches and can be disabled with CURSOR_SERVER_WEBFETCH=0.',
    },
